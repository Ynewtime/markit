...
- **全部默认模型刷新到当前代，并加了防止再次落后的守护**：八个默认里有两个定于 2026-10-15 退役，另两个落后一代且继任者更便宜——`openai/gpt-5.6-luna` 比它替换掉的 `gpt-5.4-nano` 还省钱。此前没人发现，因为陈旧的默认值只有在模型开始拒绝服务时才会出错，而那已远晚于本该更换的时点。新增守护测试：任一默认模型距离 litellm 记录的退役日期不足 120 天即失败
