...
| `--llm-batch` | Directory batches only: run the LLM stage through the provider's Batch API at half price. Needs a single-model OpenAI pool. Waits up to `--llm-batch-timeout` (default 1h), then hands off |
