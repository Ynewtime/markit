...
- **Every default model refreshed to the current generation, with a guard against the next drift**: two of the eight defaults were scheduled to retire on 2026-10-15 and two more were a generation behind a cheaper successor — `openai/gpt-5.6-luna` costs less than the `gpt-5.4-nano` it replaces. Nothing noticed, because a stale default only fails once the model starts refusing, which is long after it should have been changed. A guard test now fails when any default is within 120 days of the retirement date litellm records for it
