...
- **`markitai-mcp` brings conversion to MCP agents**: a separate package exposing `convert_document`, `convert_url`, `batch_convert` and `job_status` over stdio — once it is on PyPI, `claude mcp add markitai -- uvx markitai-mcp` is the whole setup. Results are written to disk with a truncated inline preview so a large document never floods an agent's context, and LLM enhancement stays off unless the caller asks for it
