...
Requires a single-model OpenAI or Anthropic pool. Cache hits are served instantly; documents whose batch request fails are re-run live, so a partial batch never loses output. On OpenAI, reasoning models run with reasoning off inside a batch — the batch deployments require that of function-tool calls. Not yet combinable with `--alt`/`--desc`/`--screenshot`/`--ocr` (run without `--llm-batch` for those).
