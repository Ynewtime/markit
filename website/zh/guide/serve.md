...
所有转换选项都收在**选项**里，所以默认界面什么都不问。输入区是一张卡片：URL 输入框，加上「选项」、文件上传、「转换」组成的操作行（桌面端同行排布，手机上 URL 区在操作行上方）。展开选项、或按下旁边的 CLI 按钮时，会显示与当前设置等价的命令行。语言和主题共用页头的「外观」菜单。

面板按组而非按条排列：预设在最前，**增强**收纳 LLM、OCR 与图片分析，**输出**放输出格式，**高级**折叠内容源、URL/文件抓取选择器以及缓存与压缩开关——其中任一项已是非默认值时会自动展开。
...
启动时服务会生成一个随机访问令牌，并打印形如 `http://192.168.1.50:3600/#token=mk_…` 的可直接打开的 URL。令牌位于 URL fragment 中，因此不会进入服务端访问日志。本机请求永远不需要令牌；其他机器的请求必须携带它——网页界面从 fragment 读取令牌并存入 `sessionStorage`，浏览器会把它从地址栏抹掉。脚本则发送 `Authorization: Bearer <token>`（下载/SSE 链接仍可使用 `?token=`）。设置 `MARKITAI_SERVE_TOKEN` 可在重启间固定令牌；传入 `--no-auth` 可完全禁用令牌，此时其他机器提交的 URL 目标限定为公网地址，且无法访问 LLM 设置；文件上传、历史访问、下载和删除仍然可用。

未认证远程任务的公网目标检查也覆盖 HTTP 重定向、图片下载和浏览器子请求。连接使用已验证的 IP 并保留原始 TLS 主机名；受限浏览器会话禁用 Service Worker 和 WebSocket，不复用可信会话或抓取缓存。
...
如果你通过 DNS 名称而非 IP 访问，再加上 `--allowed-host my-box.lan`：服务只接受 `Host`/`Origin` 为 localhost、IP 字面量或白名单主机名的请求；其他 DNS 名称一律拒绝，从而阻止恶意网页通过 DNS 重绑定攻击从你的浏览器访问 API。
...
- **输入区**：拖入文件或文件夹，或粘贴 URL。选项与 CLI 预设一致——`minimal`、`standard`、`rich`——外加独立的 LLM 与 OCR 开关，以及输出格式（`rag`、`obsidian`、`okf`）。输出格式开不开 LLM 都能选。
...
预设是五项功能的组合：内置 `minimal` 全部关闭，`standard` 开启 LLM、alt 文本与描述 JSON，`rich` 再加页截图；三者默认都不开 OCR。服务端配置可以覆盖这些定义。选择预设会重置这五项，单独调整则保留其它选择：例如 minimal 加 LLM 显示**自定义**，不会自动开启图片分析。五项完全匹配时高亮对应预设；这只是显示状态，不会改写底层预设与覆盖值。
...
每项任务在重启后保留自己的完整转换选项；不传覆盖参数的重试沿用该项已保存的选项。恢复快照时也会移除被其他客户端删除的条目。
