...
![markitai 网页工作台：「选项」和「上传」位于 URL 输入框上方，下方始终显示等效 CLI 命令。](/workbench.zh.png)

所有转换选项都收在**选项**里——LLM 增强、OCR、预设、输出格式、图片分析、内容源、抓取策略与后端——所以默认界面什么都不问。「选项」和「上传」位于输入框上方，无论选项是否展开，下方都会显示与当前设置等价的 CLI 命令。语言和主题共用页头的「外观」菜单。
