...
![markitai 网页工作台：文件与 URL 的拖放区，下方是「选项」与 CLI 两个折叠入口。](/workbench.zh.png)

所有转换选项都收在**选项**里——LLM 增强、OCR、预设、输出格式、图片分析、内容源、抓取策略与后端——所以默认界面什么都不问。按钮上会显示当前生效的选项摘要，**CLI** 则给出与当前设置等价的命令。
